...
bool render_t::create_window()
{
    ...
    detail->window = CreateWindowExA(
        running ? (WS_EX_TOPMOST | WS_EX_TOOLWINDOW | WS_EX_LAYERED) : (WS_EX_TOPMOST | WS_EX_TRANSPARENT | WS_EX_LAYERED | WS_EX_TOOLWINDOW),
...
void render_t::render_menu()
{
    ...
    if (add_tab("Shot Detection", 6, selected_tab_index == 6, 9))
    ...
    case 6:
    {
        ...
        ImGui::BeginChild("Shot Detection", ImVec2(ImGui::GetContentRegionAvail().x / 2 - 9.f, ImGui::GetContentRegionAvail().y - 13.f), true);

        ImGui::Checkbox("Enable Detection", &settings::shot_detection::enabled);
        if (settings::shot_detection::enabled)
        {
            ImGui::SameLine();
            inline_keybind_button("trigger_keybind", &settings::shot_detection::trigger_key);
        }

        ImGui::Spacing();
        ImGui::Text("Selection Keybind");
        ImGui::SameLine();
        ...
        if (settings::shot_detection::db_revolver_combo) {
            ...
            int current_db_slot = settings::shot_detection::db_slot_key - '1';
            if (current_db_slot < 0 || current_db_slot > 8) current_db_slot = 0;
            if (ImGui::Combo("DB Weapon Slot", &current_db_slot, slots, IM_ARRAYSIZE(slots))) {
                settings::shot_detection::db_slot_key = '1' + current_db_slot;
            }

            int current_rev_slot = settings::shot_detection::revolver_slot_key - '1';
            if (current_rev_slot < 0 || current_rev_slot > 8) current_rev_slot = 1;
            if (ImGui::Combo("Revolver Weapon Slot", &current_rev_slot, slots, IM_ARRAYSIZE(slots))) {
                settings::shot_detection::revolver_slot_key = '1' + current_rev_slot;
            }
        ...
        ImGui::SetCursorPos(ImVec2(ImGui::GetWindowWidth() * 0.5f + 8.f, 78.f));
        ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(8, 8));
        ImGui::BeginChild("Target Status", ImVec2(ImGui::GetContentRegionAvail().x - 13.f, ImGui::GetContentRegionAvail().y - 13.f), true);

        ImGui::Text("Selected Target:");
        ImGui::SameLine();
        ImGui::TextColored(menu::accent_color, settings::shot_detection::target_name.c_str());

        ImGui::Spacing();
        
        static char ammo_input_buf[64] = "";
        if (ammo_input_buf[0] == '\0') {
