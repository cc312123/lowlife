...
void SmartChangeDetectionLoop() {
    ...
    while (AutoRescan::active) {
        try {
            ...
            if (currentState.IsValid() && previousState.IsValid()) {
                if (currentState.gameId != previousState.gameId || currentState.placeId != previousState.placeId || currentState.processId != previousState.processId) {
                    ForceRescan();
