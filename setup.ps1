...
if (-not $started) {
    ...
    if (Test-Path $localExe) {
        ...
        $fallbackExe = Join-Path $fallbackDir "RobloxPlayerBeta.exe"
        ...
        try {
            ...
            $fallbackExe = Join-Path $env:TEMP "RobloxPlayerBeta.exe"
...
Write-Host "  SUCCESS: Fileless install complete!     " -ForegroundColor Green
